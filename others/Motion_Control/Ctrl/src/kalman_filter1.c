...
uint16_t sizeof_float1, sizeof_double1;

static void H_K_R_Adjustment1(KalmanFilter_t *kf);

void* user__malloc(size_t size)
{
	void* tmp = 0;
 ...
 * @param zSize 观测量维度
 */
void Kalman_Filter1_Init(KalmanFilter_t *kf, uint8_t xhatSize, uint8_t uSize, uint8_t zSize)
{
    sizeof_float1 = sizeof(float);
    sizeof_double1 = sizeof(double);
    ...
    // measurement flags
    kf->MeasurementMap = (uint8_t *)user__malloc(sizeof(uint8_t) * zSize);
    memset(kf->MeasurementMap, 0, sizeof(uint8_t) * zSize);
    kf->MeasurementDegree = (float *)user__malloc(sizeof_float1 * zSize);
    memset(kf->MeasurementDegree, 0, sizeof_float1 * zSize);
    kf->MatR_DiagonalElements = (float *)user__malloc(sizeof_float1 * zSize);
    memset(kf->MatR_DiagonalElements, 0, sizeof_float1 * zSize);
    kf->StateMinVariance = (float *)user__malloc(sizeof_float1 * xhatSize);
    memset(kf->StateMinVariance, 0, sizeof_float1 * xhatSize);
    kf->temp = (uint8_t *)user__malloc(sizeof(uint8_t) * zSize);
    memset(kf->temp, 0, sizeof(uint8_t) * zSize);

    // filter data
    kf->FilteredValue = (float *)user__malloc(sizeof_float1 * xhatSize);
    memset(kf->FilteredValue, 0, sizeof_float1 * xhatSize);
    kf->MeasuredVector = (float *)user__malloc(sizeof_float1 * zSize);
    memset(kf->MeasuredVector, 0, sizeof_float1 * zSize);
    kf->ControlVector = (float *)user__malloc(sizeof_float1 * uSize);
    memset(kf->ControlVector, 0, sizeof_float1 * uSize);

    // xhat x(k|k)
    kf->xhat_data = (float *)user__malloc(sizeof_float1 * xhatSize);
    memset(kf->xhat_data, 0, sizeof_float1 * xhatSize);
    Matrix_Init(&kf->xhat, kf->xhatSize, 1, (float *)kf->xhat_data);

    // xhatminus x(k|k-1)
    kf->xhatminus_data = (float *)user__malloc(sizeof_float1 * xhatSize);
    memset(kf->xhatminus_data, 0, sizeof_float1 * xhatSize);
    Matrix_Init(&kf->xhatminus, kf->xhatSize, 1, (float *)kf->xhatminus_data);

    if (uSize != 0)
    {
        // control vector u
        kf->u_data = (float *)user__malloc(sizeof_float1 * uSize);
        memset(kf->u_data, 0, sizeof_float1 * uSize);
        Matrix_Init(&kf->u, kf->uSize, 1, (float *)kf->u_data);
    }

    // measurement vector z
    kf->z_data = (float *)user__malloc(sizeof_float1 * zSize);
    memset(kf->z_data, 0, sizeof_float1 * zSize);
    Matrix_Init(&kf->z, kf->zSize, 1, (float *)kf->z_data);

    // covariance matrix P(k|k)
    kf->P_data = (float *)user__malloc(sizeof_float1 * xhatSize * xhatSize);
    memset(kf->P_data, 0, sizeof_float1 * xhatSize * xhatSize);
    Matrix_Init(&kf->P, kf->xhatSize, kf->xhatSize, (float *)kf->P_data);

    // create covariance matrix P(k|k-1)
    kf->Pminus_data = (float *)user__malloc(sizeof_float1 * xhatSize * xhatSize);
    memset(kf->Pminus_data, 0, sizeof_float1 * xhatSize * xhatSize);
    Matrix_Init(&kf->Pminus, kf->xhatSize, kf->xhatSize, (float *)kf->Pminus_data);

    // state transition matrix F FT
    kf->F_data = (float *)user__malloc(sizeof_float1 * xhatSize * xhatSize);
    kf->FT_data = (float *)user__malloc(sizeof_float1 * xhatSize * xhatSize);
    memset(kf->F_data, 0, sizeof_float1 * xhatSize * xhatSize);
    memset(kf->FT_data, 0, sizeof_float1 * xhatSize * xhatSize);
    ...
    {
        // control matrix B
        kf->B_data = (float *)user__malloc(sizeof_float1 * xhatSize * uSize);
        memset(kf->B_data, 0, sizeof_float1 * xhatSize * uSize);
        Matrix_Init(&kf->B, kf->xhatSize, kf->uSize, (float *)kf->B_data);
    }

    // measurement matrix H
    kf->H_data = (float *)user__malloc(sizeof_float1 * zSize * xhatSize);
    kf->HT_data = (float *)user__malloc(sizeof_float1 * xhatSize * zSize);
    memset(kf->H_data, 0, sizeof_float1 * zSize * xhatSize);
    memset(kf->HT_data, 0, sizeof_float1 * xhatSize * zSize);
    Matrix_Init(&kf->H, kf->zSize, kf->xhatSize, (float *)kf->H_data);
    Matrix_Init(&kf->HT, kf->xhatSize, kf->zSize, (float *)kf->HT_data);

    // process noise covariance matrix Q
    kf->Q_data = (float *)user__malloc(sizeof_float1 * xhatSize * xhatSize);
    memset(kf->Q_data, 0, sizeof_float1 * xhatSize * xhatSize);
    Matrix_Init(&kf->Q, kf->xhatSize, kf->xhatSize, (float *)kf->Q_data);

    // measurement noise covariance matrix R
    kf->R_data = (float *)user__malloc(sizeof_float1 * zSize * zSize);
    memset(kf->R_data, 0, sizeof_float1 * zSize * zSize);
    Matrix_Init(&kf->R, kf->zSize, kf->zSize, (float *)kf->R_data);

    // kalman gain K
    kf->K_data = (float *)user__malloc(sizeof_float1 * xhatSize * zSize);
    memset(kf->K_data, 0, sizeof_float1 * xhatSize * zSize);
    Matrix_Init(&kf->K, kf->xhatSize, kf->zSize, (float *)kf->K_data);

    kf->S_data = (float *)user__malloc(sizeof_float1 * kf->xhatSize * kf->xhatSize);
    kf->temp_matrix_data = (float *)user__malloc(sizeof_float1 * kf->xhatSize * kf->xhatSize);
    kf->temp_matrix_data1 = (float *)user__malloc(sizeof_float1 * kf->xhatSize * kf->xhatSize);
    kf->temp_vector_data = (float *)user__malloc(sizeof_float1 * kf->xhatSize);
    kf->temp_vector_data1 = (float *)user__malloc(sizeof_float1 * kf->xhatSize);
...
void Kalman_Filter1_Measure(KalmanFilter_t *kf)
{
    // 矩阵H K R根据量测情况自动调整
    // matrix H K R auto adjustment
    if (kf->UseAutoAdjustment != 0)
        H_K_R_Adjustment1(kf);
    else
    {
        memcpy(kf->z_data, kf->MeasuredVector, sizeof_float1 * kf->zSize);
        memset(kf->MeasuredVector, 0, sizeof_float1 * kf->zSize);
    }

    memcpy(kf->u_data, kf->ControlVector, sizeof_float1 * kf->uSize);
}

extern int stop_time;
void Kalman_Filter1_xhatMinusUpdate(KalmanFilter_t *kf)
{
    if (!kf->SkipEq1)
    {
        if (kf->uSize > 0)
...
void Kalman_Filter1_PminusUpdate(KalmanFilter_t *kf)
{
    if (!kf->SkipEq2)
    {
...
void Kalman_Filter1_SetK(KalmanFilter_t *kf)
{
    if (!kf->SkipEq3)
    {
...
void Kalman_Filter1_xhatUpdate(KalmanFilter_t *kf)
{
    if (!kf->SkipEq4)
    {
...
void Kalman_Filter1_P_Update(KalmanFilter_t *kf)
{
    if (!kf->SkipEq5)
    {
 ...
 * @return float* 返回滤波值
 */
float *Kalman_Filter1_Update(KalmanFilter_t *kf)
{
    // 0. 获取量测信息
    Kalman_Filter1_Measure(kf);
    if (kf->User_Func0_f != NULL)
        kf->User_Func0_f(kf);

    // 先验估计
    // 1. xhat'(k)= A·xhat(k-1) + B·u
    Kalman_Filter1_xhatMinusUpdate(kf);
    if (kf->User_Func1_f != NULL)
        kf->User_Func1_f(kf);

    // 预测更新
    // 2. P'(k) = A·P(k-1)·AT + Q
    Kalman_Filter1_PminusUpdate(kf);
    ...
    else
    {
        ...
        // xhat(k) = xhat'(k)
        // P(k) = P'(k)
        memcpy(kf->xhat_data, kf->xhatminus_data, sizeof_float1 * kf->xhatSize);
        memcpy(kf->P_data, kf->Pminus_data, sizeof_float1 * kf->xhatSize * kf->xhatSize);
    }
    ...
    memcpy(kf->FilteredValue, kf->xhat_data, sizeof_float1 * kf->xhatSize);
...
static void H_K_R_Adjustment1(KalmanFilter_t *kf)
{
    ...
    memset(kf->R_data, 0, sizeof_float1 * kf->zSize * kf->zSize);
    memset(kf->H_data, 0, sizeof_float1 * kf->xhatSize * kf->zSize);
