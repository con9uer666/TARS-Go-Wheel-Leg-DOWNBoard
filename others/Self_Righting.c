...
/* 第一阶段（伸腿）目标与判定参数 */
float g_sr_target_l0 = 0.44f;          /* 目标腿长（用户要求默认0.44）。 */
float g_sr_l0_reached_tol = 0.01f;    /* 到达目标腿长的误差阈值。 */
float g_sr_l0_ctrl_ramp_rate = 0.04f;   /* 伸腿力斜坡速率；0表示不用斜坡。 */
float g_sr_l0_ctrl_f0_max = 150.0f;    /* 伸腿力F0上限。 */
float g_sr_l0_stuck_thresh = 0.01f;   /* 伸腿阶段卡住判定阈值（腿长变化死区）。 */

/* 第一阶段卡住时“换角度再伸腿”参数 */
float g_sr_extend_unstuck_speed_l = -2.0f;      /* 左腿解卡转速指令（算绝对值） */
float g_sr_extend_unstuck_speed_r = 2.0f;     /* 右腿解卡转速指令（算绝对值） */
float g_sr_extend_unstuck_torque_max = 10.0f;   /* 解卡时最大转矩。 */
...
float g_sr_extend_passive_damping_torque_max = 3.0f;   /* 被动阻尼转矩限幅。 */

/* 第二阶段（反向匀速转到并齐）参数 */
float g_sr_reverse_speed_l = -4.0f;        /* 左腿反向匀速转速度目标（方向由符号决定，用户可调）。 */
float g_sr_reverse_speed_r = -4.0f;        /* 右腿反向匀速转速度目标（方向由符号决定，用户可调）。 */
float g_sr_reverse_torque_max = 8.0f;      /* 第二阶段转矩上限（用户可调，且不会超过该值）。 */
float g_sr_reverse_torque_ramp = 0.0f;     /* 第二阶段转矩斜坡速率。 */
float g_sr_turn_stuck_thresh = 0.002f;     /* 第二阶段转动卡住判定阈值。 */
float g_sr_align_tol = 0.02f;              /* 两腿并齐角度误差阈值（|phiL-phiR| <= tol）。 */   //TODO: 调参

/* 第三阶段（大力矩匀速转到目标角）参数 */
float g_sr_target_angle_l = 0.0f;          /* 左腿目标角度（大地坐标系 b_phi0）。 */
float g_sr_target_angle_r = 0.0f;          /* 右腿目标角度（大地坐标系 b_phi0）。 */
float g_sr_target_angle_tol = 0.02f;       /* 达到目标角度的误差阈值。 */
float g_sr_sync_speed = 5.0f;              /* 并齐后同步转的基准匀速。 */
float g_sr_sync_fast_speed = 6.0f;         /* 未并齐且卡住进入第三阶段时，远端腿较快速度。 */
float g_sr_sync_slow_speed = 2.0f;         /* 未并齐且卡住进入第三阶段时，近端腿较慢速度。 */
float g_sr_sync_torque_max = 18.0f;        /* 第三阶段大力矩上限。 */
 ...
 * @return float 
 */
static float sr_clamp_abs(float value, float max_mag)
{
	if (max_mag < 0.0f)
	{
		max_mag = -max_mag;
...
uint8_t Self_Righting_Step(void)
{
	...
	float rem_l;//非负数
	float rem_r;//非负数
	int reached_ang_l;//左腿到达目标角标签，1：到达；0：未达到
	int reached_ang_r;//右腿到达目标角标签，1：到达；0：未达到
    ...
    phi_diff = VMC_L.b_phi0 - VMC_R.b_phi0;
	...
	if (g_self_righting_stage == SELF_RIGHTING_STAGE_EXTEND)
	{
		...
		l0_stuck_l = leg_length_stuck_detect(&VMC_L, g_sr_l0_stuck_thresh);
		l0_stuck_r = leg_length_stuck_detect(&VMC_R, g_sr_l0_stuck_thresh);
		...
		else//不卡
		{
            ...
            t_l = -g_sr_extend_passive_damping * VMC_L.d_b_phi0;
			t_l = sr_clamp_abs(t_l, g_sr_extend_passive_damping_torque_max);    //TODO: 这个限幅可以给大点，调参
		}
		...
		{
			t_r = -g_sr_extend_passive_damping * VMC_R.d_b_phi0;
			t_r = sr_clamp_abs(t_r, g_sr_extend_passive_damping_torque_max);
		}
	...
	else if (g_self_righting_stage == SELF_RIGHTING_STAGE_REVERSE_TURN)
	{
		...
		t_l = leg_turn_speed_control(&VMC_L, g_sr_reverse_speed_l, g_sr_reverse_torque_max, g_sr_reverse_torque_ramp);//TODO: 调参，要不要斜坡
		...
		aligned = (fabsf(VMC_L.b_phi0 - VMC_R.b_phi0) <= g_sr_align_tol) ? 1 : 0;
		turn_stuck_l = leg_turn_stuck_detect(&VMC_L, g_sr_turn_stuck_thresh);
		turn_stuck_r = leg_turn_stuck_detect(&VMC_R, g_sr_turn_stuck_thresh);
	...
	else if (g_self_righting_stage == SELF_RIGHTING_STAGE_SYNC_HIGH_TORQUE)
	{
		...
		rem_l = fabsf(g_sr_target_angle_l - VMC_L.b_phi0);
		rem_r = fabsf(g_sr_target_angle_r - VMC_R.b_phi0);
		...
		if (g_self_righting_sync_from_stuck == 0U)
		{
			...
			if (!reached_ang_l)
			{
				cmd_spd_l = get_sign_float(g_sr_target_angle_l - VMC_L.b_phi0) * fabsf(g_sr_sync_speed);
			}
			if (!reached_ang_r)
			{
				cmd_spd_r = get_sign_float(g_sr_target_angle_r - VMC_R.b_phi0) * fabsf(g_sr_sync_speed);
			}
		}
		else//未并齐但卡住进入第三阶段，使用差速策略
		{
			...
			if (!reached_ang_r)
			{
				float cmd_spd_r = fabsf(g_sr_sync_fast_speed) * (rem_r / max_rem);
				if (cmd_spd_r < fabsf(g_sr_sync_slow_speed))
				{
		...
		t_l = leg_turn_speed_control(&VMC_L, cmd_spd_l, g_sr_sync_torque_max, g_sr_sync_torque_ramp);
		t_r = leg_turn_speed_control(&VMC_R, cmd_spd_r, g_sr_sync_torque_max, g_sr_sync_torque_ramp);
