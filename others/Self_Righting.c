...
/* 第一阶段卡住时“换角度再伸腿”参数 */
float g_sr_extend_unstuck_speed_l = 0.5f;      /* 左腿解卡转速指令（算绝对值） */
float g_sr_extend_unstuck_speed_r = 0.5f;     /* 右腿解卡转速指令（算绝对值） */
...
uint8_t Self_Righting_Step(void)
{
	...
	if ((g_self_righting_stage == SELF_RIGHTING_STAGE_EXTEND && SR_test_state == 0) || (SR_test_state == 1))
	{
		...
		l0_stuck_l = leg_length_stuck_detect(&VMC_L, g_sr_l0_stuck_thresh, 0.5f);
		l0_stuck_r = leg_length_stuck_detect(&VMC_R, g_sr_l0_stuck_thresh, 0.5f);
