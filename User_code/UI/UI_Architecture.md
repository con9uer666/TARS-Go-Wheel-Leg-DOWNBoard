...
30HZ     _1       ui_5_frame_t        (0, 0,  8 / 9 / 13)   ← 3 活跃 + 2 空槽
30HZ     _FAST    ui_5_frame_t        (0, 0,  7 / 10..12)   ← 4 活跃 + 1 空槽，姿态高刷
...
> **30HZ_1 / 30HZ_FAST 的 ID 不连续**：因为 FAST 子帧是把原 30HZ_1 的 7 个图元
> 按"姿态可视化 vs 心跳/电量"拆分后形成的。两边各自保留原始 ID（7/10/11/12
> 给 FAST，8/9/13 给新 30HZ_1），跨固件升级时客户端不会出现"旧 ID 残留为
> 旧图元类型"的幽灵。
...
如果每 tick 把全部 5 个 30HZ 子帧 + 2 个 5HZ 子帧一次性发完，启动期就会瞬间超限被裁判系统过滤丢包。

### 7.2 调度表（按图元刷新优先级分配 slot）

主循环以 **10Hz** 心跳（`osDelay(100)`），用 `cnt % 10` 按"图元越关键 → slot 越多"分配带宽：

```
 tick | cnt%10 | 发送子帧       | 实际刷新率 | 内容
─────┼────────┼───────────────┼───────────┼─────────────────────────
   0 |   0    | 30HZ_FAST     | 4 Hz      | 车身朝向弧 / 双腿 / pitch
   1 |   1    | 30HZ_0        | 2 Hz      | 数字 (FRIC/AIM/SHOOT) + 左 8009
   2 |   2    | 30HZ_1        | 1 Hz      | 右 8009 / 超电
   3 |   3    | 30HZ_FAST     ◀
   4 |   4    | 30HZ_2        | 1 Hz      | 功率/485/blink/左 3508
   5 |   5    | 30HZ_0        ◀
   6 |   6    | 30HZ_FAST     ◀
   7 |   7    | 30HZ_3        | 1 Hz      | 右 3508 / PITCH/ROLL/摩擦轮
   8 |   8    | 30HZ_FAST     ◀
   9 |   9    | 5HZ 文字      | on-change | PLEASE SPIN / LONG LEG（见 7.4）
```

**关键洞察**：
- **FAST 子帧拿 4 个 slot = 4Hz**：姿态可视化的肉眼效果最敏感，给到最高刷新；
- **30HZ_0 拿 2 个 slot = 2Hz**：数字（摩擦轮转速/弹数/自瞄）变化快但精度容忍度高；
- **其它 30HZ 子帧各 1 个 slot = 1Hz**：基本都是心跳灯（width 切换）和电量条，1Hz 足够；
- **5HZ 文字共享 1 个 slot**：文字提示状态变化稀疏（按操作模式切换），on-change 策略
  能把延迟压到 ≤1s，比固定 1Hz 还快；同时省下的 slot 喂给 FAST。

总包数恰好 10 pps，**贴着上限但绝不超过**。

### 7.3 30HZ_1 与 30HZ_FAST 的拆分动机

老版本里 `30HZ_1` 把 7 个图元混在一个 7-frame 里，整个子帧只能拿到 2Hz，
姿态可视化（车身朝向/双腿/pitch 直线）肉眼看像在卡顿。

重构后按"姿态可视化"vs"低频心跳/电量"切成两条子帧，各自独立调度：

| 子帧        | 图元                                   | 历史 ID         | 刷新率 |
|------------|---------------------------------------|----------------|--------|
| `30HZ_FAST` | BODY_FRONT / L_LEG / R_LEG / BODY_PITCH | 7 / 10 / 11 / 12 | 4 Hz   |
| `30HZ_1`    | 8009RF / SUPER_CUP / 8009RB           | 8 / 9 / 13      | 1 Hz   |

两条子帧都用 5-frame（5 槽位）承载，留 1-2 个空槽不发图元（`operate_type=0`）。
代价是浪费 15B × 1~2 = 15~30B 的协议带宽，换取的是不必新增 1-frame/3-frame 类型，
保持类型层简洁。

### 7.4 5HZ 文字共享 slot：on-change 优先 + 交替兜底

5HZ 文字的状态变化稀疏（玩家切换底盘 / 腿态模式时才变），如果死板地每 0.5Hz 各发一次，
slot 9 大部分时候在发"重复"包。改成 on-change 策略后逻辑变成：

```
slot 9 触发：
    若 PLEASE SPIN 状态变了 → 发 PLEASE SPIN（消费变化）
    否则若 LONG LEG 状态变了 → 发 LONG LEG
    否则 → 按 alt 交替重发（每条 0.5Hz）兜底丢包
    若两条都变了 → 按 alt 先发一条，另一条等下一个 slot 9（1s 后）
```

效果：
- **状态变化延迟 ≤1s**（任意时刻状态变更，最坏等到下一个 slot 9）；
- **稳态下每条文字 0.5Hz 重发**，足以对抗偶发丢包；
- **不超 1 pps**（slot 9 永远只发 1 个包）。
...
sequenceDiagram
    ...
    Note over Task: osDelay(50)
    Task->>Client: ui_init_g_30HZ() (5 子帧, 共 26 图元)
    Note over Task: osDelay(50)
...
**对策**：每 30 个 tick (= 3s) 调用一次 `ui_update_g_INIT()`，把所有 INIT 图元以 `operate_type=1`(新增) 重发。

> 注意：必须用 **新增**(1) 而不是 **修改**(2)。`operate_type=2` 对客户端上"已不存在"的图元是 no-op —— 一旦因为丢包/瞬时高负载（如 `cnt%30==0` 那拍 30HZ_0 + 4 个 INIT 子帧瞬时超过 10pps 上限）让某个静态图元从客户端消失，用 `2` 永远救不回来。表现就是：操作手屏幕上 "FRIC SPD" / "AUTO AIM" / "SHOOT NUM" 标签字偶发性永久缺失，直到下电重启。改成 `1` 后客户端把"新增已存在图元"当作幂等替换，自愈成功。
...
| 调度 | 单帧轮询 10pps，按优先级分配 slot | 低优先级子帧降到 1Hz |
| 姿态可视化拆 FAST 子帧 | 4Hz 高刷 | 占 4 个 slot，挤占其它子帧带宽 |
| 5HZ 文字共享 1 slot | on-change + 交替兜底 | 极端"双变化同发"时其中一条延迟 1s |
